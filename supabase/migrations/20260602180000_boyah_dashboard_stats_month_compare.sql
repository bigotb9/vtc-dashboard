-- ============================================================
-- COMPARAISON MENSUELLE Yango : mois courant vs mois precedent
-- 02/06/2026
-- ============================================================
-- Ajout PUREMENT ADDITIF a boyah_dashboard_stats : on expose le CA du mois
-- calendaire PRECEDENT complet (revenue.prevMonth) et l'evolution mois/mois
-- (revenue.trendMonthPct), plus la commission Boyah du mois precedent
-- (commission.prevMonth). Objectif : BoyahBot peut dire "CA Yango ce mois vs
-- mois precedent, +X%" (re-expose ca_prestataires_mois_prec / evolution_mois_pct
-- cote agent/process, perdus quand fetchTransport est passe sur cette RPC).
--
-- ⚠️ AUCUN champ existant n'est modifie. week / prevWeek / month / today /
-- total / trendWeekPct / especes / sanEspeces / totals / charts / topDrivers /
-- topVehicles / commission.{today,week,month,total} restent IDENTIQUES. Les
-- consommateurs actuels (route dashboard-stats, qui renvoie le jsonb tel quel)
-- ne lisent que des champs inchanges -> non impactes. La route driver-stats
-- utilise une fonction DIFFERENTE (boyah_driver_stats) -> hors de portee.
--
-- Bornes (calendaires, fuseau Abidjan = UTC+0) :
--   month_start      = date_trunc('month', now())            (1er du mois courant)
--   month_prev_start = date_trunc('month', now()) - 1 month  (1er du mois precedent)
--   month (existant) = SUM price complete WHERE d >= month_start         (1er -> auj. inclus, PARTIEL en debut de mois)
--   prevMonth (new)  = SUM price complete WHERE d >= month_prev_start AND d < month_start (mois precedent COMPLET)
--   trendMonthPct    = round((month - prevMonth)/prevMonth*100), NULL si prevMonth=0
--
-- ⚠️ COHERENCE A DOCUMENTER : `month` est le mois courant PARTIEL (1er ->
-- aujourd'hui), `prevMonth` est le mois precedent COMPLET. En debut de mois,
-- comparer 2 jours de juin a tout mai donne une "baisse" apparente NORMALE
-- (meme logique que trendWeekPct). BoyahBot est averti via le system prompt de
-- ne pas s'alarmer d'une baisse mensuelle apparente en debut de mois.
--
-- CREATE OR REPLACE : idempotent, lecture seule, reversible (re-appliquer la
-- migration 20260602140000 retire prevMonth/trendMonthPct).
-- ============================================================
