-- ============================================================
-- FIX 504 /api/boyah-transport/dashboard-stats
-- 02/06/2026
-- ============================================================
-- La route chargeait les ~65 000 lignes de commandes_yango (colonne raw
-- jsonb incluse) en ~65 requetes paginees sequentielles, puis agregait en
-- JS (>100 passes sur 64K lignes) -> depassait maxDuration=30s -> 504.
--
-- Cette fonction pousse TOUTE l'agregation dans Postgres : un seul scan
-- materialise de la table, extraction jsonb une seule fois, puis group by.
-- La route ne fait plus qu'un appel RPC et renvoie le jsonb tel quel.
--
-- Perimetre : TOUT le compte Yango (dashboard operationnel plateforme
-- partenaire), identique a l'ancienne route.
--
-- Chemins jsonb (confirmes par cartographie B1) :
--   prix      : (raw->>'price')::numeric        ("800.0000", FCFA entiers)
--   chauffeur : raw->'driver_profile'->>'id' / ->>'name'
--   vehicule  : raw->'car'->>'id' (cle) / ->>'callsign' (PLAQUE) / ->>'brand_model'
--               (callsign CORRIGE le bug : l'ancienne route lisait car->>'number'
--                qui n'existe pas -> top vehicules regroupait par modele)
--   paiement  : raw->>'payment_method'  ('cash' = especes ; sinon sans especes)
--   statut    : colonne status (complete / cancelled)
--   dates     : colonnes created_at / ended_at (indexees), Abidjan = UTC+0
--
-- Index : AUCUN nouvel index requis. Les totaux sont "all-time" -> un seul
-- scan sequentiel complet est de toute facon necessaire et optimal ; un index
-- expression sur driver/car id ne serait pas utilise par un GROUP BY full-table.
--
-- SECURITY INVOKER (defaut) + GRANT EXECUTE : la route appelle avec la cle
-- anon (commandes_yango n'a pas de RLS, le SELECT anon fonctionne deja).
-- ============================================================
...
agg AS (
  SELECT
    ...
    coalesce(sum(b.price) FILTER (WHERE b.is_complete AND b.d >= p.today - 7), 0)                       AS rev_week,
    coalesce(sum(b.price) FILTER (WHERE b.is_complete AND b.d >= p.today - 14 AND b.d < p.today - 7), 0) AS rev_prevweek,
    coalesce(sum(b.price) FILTER (WHERE b.is_complete AND date_trunc('month', b.d)::date = p.month_start), 0) AS rev_month,
