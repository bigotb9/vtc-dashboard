...
export async function POST() {
  try {
    ...
    // numero_wave_2 / numero_wave_3 = numéros secondaires (autres comptes Wave du même chauffeur)
    const { data: chauffeurs } = await sb
      .from("chauffeurs")
      .select("id_chauffeur, numero_wave, numero_wave_2, numero_wave_3")
    ...
    // Supporte jusqu'à 3 numéros Wave par chauffeur (numero_wave + numero_wave_2 + numero_wave_3).
    // Si un chauffeur a un ou plusieurs numéros secondaires (comptes Wave différents),
    // ses versements faits depuis ces numéros seront bien rattachés.
    ...
    for (const c of chauffeurs || []) {
      ...
      const p3 = normPhone8((c as Record<string, unknown>).numero_wave_3 as string)
      if (p3) chByPhone.set(p3, c.id_chauffeur)
    }
