 ...
 *
 * Body : { exercice_id }
 * Génère le PDF + archive dans etats_financiers_archives + retourne PDF.
 * Phase 4.2 Module 3a §6.
 */
...
import { renderBilanPdfTemplate } from "@/components/compta/pdf/BilanPdfTemplate"
...
export async function POST(req: NextRequest) {
  ...
  try {
    // ✦ PHASE 4.3 — Avant tout, (re)créer l'auto-écriture résultat (compte 13)
    //   si l'exercice est ouvert. Sur exercice clos, l'écriture est déjà figée.
    ...
    // Hash + UUID + QR de vérification (PATCH Phase 4.2)
    const uuid        = newTraceabilityUuid()
    const generatedAt = new Date().toISOString()
    const hash        = computeHashSha256({ type: "bilan", data })
    const qr          = await buildVerifyQr(uuid)

    const htmlBody = renderBilanPdfTemplate({
      data, societe,
      traceability: {
        uuid,
        hash_sha256: hash,
    ...
    // Archive (best-effort — n'échoue pas si insert KO)
    try {
      ...
      console.warn("[bilan.export-pdf] archive insert KO:", e)
    }

    const filename = `bilan-${data.exercice_libelle.toLowerCase().replace(/\s+/g, "-")}-${data.date_arrete}.pdf`
    ...
    console.error("[bilan.export-pdf]", e)
    return comptaError("INTERNAL_ERROR", { hint: (e as Error).message }, "Erreur génération PDF Bilan")
  }
}
