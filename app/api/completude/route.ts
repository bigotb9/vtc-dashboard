...
export async function GET(req: NextRequest) {
  try {
    ...
    const attribs = await fetchAll<{ id_vehicule: number; jour_exploitation: string; montant_attribue: number; type_attribution: string }>(
    ...
    const attribMap = new Map<string, { montant: number; types: string[]; count: number }>()
    for (const a of attribs) {
      const k = `${a.id_vehicule}|${a.jour_exploitation}`
      const cur = attribMap.get(k) || { montant: 0, types: [], count: 0 }
