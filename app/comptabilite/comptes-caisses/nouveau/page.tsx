...
/**
 * /comptabilite/comptes-caisses/nouveau — Écran 5 Phase 3 (création).
 */
...
export default function NouvelleCaissePage() {
  ...
  const { create, loading } = useCompteCaisseForm()
  ...
  async function handleSubmit(input: CompteCaisseFormInput) {
    ...
    if (res.ok) {
      ...
      router.push(`/comptabilite/comptes-caisses/${res.data.id}`)
    } else {
  ...
  return (
    <div className="space-y-5 max-w-4xl mx-auto">
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1.5">
            Créer un nouveau moyen de paiement avec mapping SYSCOHADA
          </p>
        </div>
      </div>
      ...
      <CompteCaisseForm
        ...
        onCancel={() => router.push("/comptabilite/comptes-caisses")}
      />
    </div>
  )
}
