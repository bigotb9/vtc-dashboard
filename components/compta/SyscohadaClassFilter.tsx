...
/**
 * Sélecteur SYSCOHADA 2 niveaux : (filtre classe) + (dropdown compte filtré).
 *
 * Variante du SyscohadaSelector qui ajoute un filtre par classe (1-9) en
 * dropdown séparé. Le compte se choisit ensuite dans la liste filtrée.
 *
 * Référence : doc Phase 3 Écran 6 §4.4.
 */
...
const CLASSES: { value: string; label: string }[] = [
  ...
  { value: "1",   label: "Cl. 1 — Ressources durables" },
  { value: "2",   label: "Cl. 2 — Immobilisations" },
  { value: "3",   label: "Cl. 3 — Stocks" },
  { value: "4",   label: "Cl. 4 — Tiers" },
  { value: "5",   label: "Cl. 5 — Trésorerie" },
  { value: "6",   label: "Cl. 6 — Charges" },
  { value: "7",   label: "Cl. 7 — Produits" },
  { value: "8",   label: "Cl. 8 — Autres ch./pr." },
  { value: "9",   label: "Cl. 9 — Comptabilité analytique" },
]

type Props = {
  value:     string | null
  onChange:  (code: string | null) => void
  /** Sens de la catégorie : aide à proposer une classe par défaut. */
  sens?:     "debit" | "credit" | null
  error?:    string | null
  required?: boolean
}

export function SyscohadaClassFilter({ value, onChange, sens, error, required }: Props) {
  const id = useId()
  ...
  // Charge la liste au premier ouverture OU quand classe change
  useEffect(() => {
    if (!open) return
    let cancelled = false
    setLoading(true)
    const params = new URLSearchParams()
    if (classe !== "all") params.set("classe", classe)
    authFetch(`/api/compta/comptes-syscohada${params.toString() ? "?" + params.toString() : ""}`)
      .then(r => r.json())
      .then(j => {
        if (cancelled) return
  ...
  // Hint sens → classe recommandée
  const hint = sens === "credit"
    ? "Pour une entrée, privilégiez les classes 7 (produits) ou 4 (tiers actif)."
    : sens === "debit"
      ? "Pour une sortie, privilégiez les classes 6 (charges) ou 4 (tiers passif)."
      : "Tout le plan SYSCOHADA est disponible — filtrez par classe pour cibler."
  ...
  return (
    <div className="relative">
      ...
      <p className="mt-1.5 text-[11px] text-gray-400 dark:text-gray-500 leading-snug">{hint}</p>
      ...
      {open && coords && (
        <div
          ...
          className="rounded-xl border border-gray-200 dark:border-white/[0.08] bg-white dark:bg-[#1a1b1f] shadow-2xl shadow-black/30 max-h-[400px] overflow-hidden flex flex-col"
        >
          {/* Bandeau filtre classe */}
          <div className="px-2 py-2 border-b border-gray-100 dark:border-white/[0.06] flex items-center gap-2 flex-wrap">
            <select
              value={classe}
              onChange={e => setClasse(e.target.value)}
              className="text-[11.5px] bg-gray-50 dark:bg-white/[0.04] border border-gray-200/70 dark:border-white/[0.08] rounded-md px-2 py-1 cursor-pointer focus:outline-none focus:ring-2 focus:ring-violet-500/30"
            >
