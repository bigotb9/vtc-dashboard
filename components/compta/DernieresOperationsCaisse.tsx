...
/**
 * Card "5 dernières opérations" sur cette caisse/compte (Écran 5 §3.6).
 * Avec lien "Voir toutes (N)" qui pousse vers la liste opérations
 * pré-filtrée par caisse_id/compte_id.
 */
...
export function DernieresOperationsCaisse({ detail }: Props) {
  const filterParam = detail.type_cible === "caisse"
    ? `caisse_id=${detail.id}`
    : `compte_id=${detail.id}`
    ...
    <div className="rounded-2xl bg-white dark:bg-white/[0.02] border border-gray-200/70 dark:border-white/[0.06] p-5 relative overflow-hidden">
      ...
      <div className="flex items-start justify-between mb-4 gap-3">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div>
            <h3 className="text-sm font-bold text-gray-900 dark:text-white">5 dernières opérations</h3>
            <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5">Validées · ordre antichronologique</p>
          </div>
        </div>
        <Link
          href={`/comptabilite/operations?${filterParam}`}
          className="text-[11px] font-semibold text-violet-600 dark:text-violet-400 hover:underline whitespace-nowrap"
        >
          Voir tout ({detail.nb_mouvements})
        </Link>
      </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10.5px] text-gray-500 dark:text-gray-400 mt-0.5 flex items-center gap-1.5 truncate">
                      ...
                      {op.journal_code && (
                        <>
                          <span>·</span>
                          <span className="font-mono">{op.journal_code}</span>
                        </>
                      )}
                      {op.categorie_libelle && (
                        <>
                          <span>·</span>
                          <span className="truncate">{op.categorie_libelle}</span>
                        </>
                      )}
                    </p>
                  </div>
